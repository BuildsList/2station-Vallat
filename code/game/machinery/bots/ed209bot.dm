...
/obj/machinery/bot/ed209/New()
	...
	spawn(3)
		...
		if(lasercolour)
			req_access = list(ACCESS_MAINT_TUNNELS_TG,ACCESS_CLOWN,ACCESS_MIME)
			arrest_type = 1
			if(lasercolour == "b")
...
/obj/item/weapon/ed209_assembly/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(((istype(W, /obj/item/robot_parts/l_leg)) || (istype(W, /obj/item/robot_parts/r_leg))) && (src.build_step == 0 || src.build_step == 1))
		...
		src.name = "vest/legs/frame assembly"
		lasercolour = "b"
		src.item_state = "[lasercolour]ed209_shell"
		src.icon_state = "[lasercolour]ed209_shell"
		del(W)
		...
		src.item_state = "[lasercolour]ed209_hat"
		src.icon_state = "[lasercolour]ed209_hat"
		del(W)
	else if(isprox(W) && (src.build_step == 5))
		src.build_step++
		user << "You add the prox sensor to [src]!"
		src.name = "prox/covered and armed the frame assembly"
		src.item_state = "[lasercolour]ed209_prox"
		src.icon_state = "[lasercolour]ed209_prox"
		del(W)
	...
	else if(istype(W, /obj/item/weapon/gun/energy/taser) && (src.build_step == 7) && (!lasercolour))
		src.build_step++
		...
		del(W)
	else if(istype(W, /obj/item/weapon/gun/energy/laser/bluetag) && (src.build_step == 7) && (lasercolour == "b"))
		src.build_step++
		...
		src.icon_state = "[lasercolour]ed209_taser"
		del(W)
	else if(istype(W, /obj/item/weapon/gun/energy/laser/redtag) && (src.build_step == 7) && (lasercolour == "r"))
		src.build_step++
		user << "You add the lasertag gun to [src]!"

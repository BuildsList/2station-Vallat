	...
	if(rebuild) // rebuild the buttons and shit
		for(var/i = 1, i <= PRODUCTS_PER_PAGE, i++) // "refresh" everything to default state
			...
			if(i % 2 == 0) // even number (remainder of i / 2 == 0)
				winset(user, "vendingwindow_n.product[i]", "background-colour=#78A6C7") // Specific even colour
			else
				winset(user, "vendingwindow_n.product[i]", "background-colour=#8BA5B4") // Odd colour
	...
	for (var/i = 1, i <= products.len, i++) // loop through all products
		...
		if(product)
			...
			if(product.amount <= 0 && product.price > points)
				...
				winset(user, "vendingwindow_n.product[i]", "background-colour=#526F7C") // Darken the button
	...
	for (i = 0, i < 6, i++)
		if (products.len >= base + i)
			...
			if (product.amount > 0)
				...
				winset(user, "vendingslot[i+1].stock", "text-colour=\"#000000\"")
				winshow(user, "vendingslot[i+1].buy", 1)
			else
				winset(user, "vendingslot[i+1].stock", "text=\"OUT OF STOCK\"")
				winset(user, "vendingslot[i+1].stock", "text-colour=\"#FF0000\"")

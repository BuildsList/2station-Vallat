...
/proc/RandomAAlarmWires()
	...
	AAlarmIndexToWirecolour = list(0, 0, 0, 0, 0)
	AAlarmWirecolourToIndex = list(0, 0, 0, 0, 0)
	var/flagIndex = 1
	for (var/flag=1, flag<32, flag+=flag)
		...
		while (!valid)
			...
			if (AAlarmwires[colourIndex]==0)
				...
				AAlarmIndexToWirecolour[flagIndex] = colourIndex
				AAlarmWirecolourToIndex[colourIndex] = flagIndex
		flagIndex+=1
	return AAlarmwires
...
//HACKING//
///////////
	proc/isWirecolourCut(var/wirecolour)
		var/wireFlag = AAlarmWirecolourToFlag[wirecolour]
	...
	proc/cut(var/wirecolour)
		var/wireFlag = AAlarmWirecolourToFlag[wirecolour]
		var/wireIndex = AAlarmWirecolourToIndex[wirecolour]
	...
	proc/mend(var/wirecolour)
		var/wireFlag = AAlarmWirecolourToFlag[wirecolour]
		var/wireIndex = AAlarmWirecolourToIndex[wirecolour] //not used in this function
		AAlarmwires |= wireFlag
		switch(wireIndex)
			if(AALARM_WIRE_IDSCAN)
	...
	proc/pulse(var/wirecolour)
		//var/wireFlag = AAlarmWirecolourToFlag[wirecolour] //not used in this function
		var/wireIndex = AAlarmWirecolourToIndex[wirecolour]
		switch(wireIndex)
	...
	attack_hand(mob/user)
		...
		if(wiresexposed && (!istype(user, /mob/living/silicon)))
			...
			for(var/wiredesc in AAlarmwires)
				var/is_uncut = AAlarmwires & AAlarmWirecolourToFlag[AAlarmwires[wiredesc]]
				t1 += "[wiredesc] wire: "
				if(!is_uncut)

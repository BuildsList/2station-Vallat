...
/obj/machinery/computer/card/attack_hand(var/mob/user as mob)
	...
	if (mode) // accessing crew manifest
		...
		if (authenticated && modify)
			var/jobs_all = ""
			var/list/alljobs = (scan.fraction == "TG" ? Get_TG_Jobs() : (scan.fraction == "MMC" ? Get_MMC_Jobs() : "")) + "Custom"
			...
			carddesc += "<b>Registered:</b> <input type='text' id='namefield' name='reg' value='[target_owner]' style='width:250px; background-colour:white;' onchange='markRed()'>"
			...
			if(istype(src,/obj/machinery/computer/card/centcom))
				...
				if (scan.fraction == "TG")
					for(var/i = 1; i <= 6; i++)
						accesses += "<td style='width:14%'><b>[get_region_accesses_name_TG(i)]:</b></td>"
					accesses += "</tr><tr>"
					for(var/i = 1; i <= 6; i++)
						accesses += "<td style='width:14%' valign='top'>"
						for(var/A in get_region_accesses_TG(i))
							if(A in modify.access)
								accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=0'><font color=\"red\">[dd_replacetext(get_access_desc_TG(A), " ", "&nbsp")]</font></a> "
							else
								accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=1'>[dd_replacetext(get_access_desc_TG(A), " ", "&nbsp")]</a> "
							accesses += "<br>"
						accesses += "</td>"
					accesses += "</tr></table>"
				if (scan.fraction == "MMC")
					for(var/i = 1; i <= 4; i++)
						accesses += "<td style='width:14%'><b>[get_region_accesses_name_Mine(i)]:</b></td>"
					accesses += "</tr><tr>"
					for(var/i = 1; i <= 4; i++)
						accesses += "<td style='width:14%' valign='top'>"
						for(var/A in get_region_accesses_Mine(i))
							if(A in modify.access)
								accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=0'><font color=\"red\">[dd_replacetext(get_access_desc_Mine(A), " ", "&nbsp")]</font></a> "
							else
								accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=1'>[dd_replacetext(get_access_desc_Mine(A), " ", "&nbsp")]</a> "
							accesses += "<br>"
						accesses += "</td>"
					accesses += "</tr></table>"

...
/datum/effect/effect/system/chem_smoke_spread
	...
	start()
		...
		if(finalcolour)
			finalcolour = rgb(rgbcolour[1], rgbcolour[2], rgbcolour[3]) // slightly darker colour
		...
		for(i=0, i<src.number, i++)
			...
			spawn(0)
				...
				if(finalcolour)
					smoke.icon += finalcolour // give the smoke colour, if it has any to begin with
				else

...
/obj/item/mecha_parts/mecha_equipment/tool/sleeper
	...
	Topic(href,href_list)
		..()
		var/datum/topic_input/filter_ = new /datum/topic_input(href,href_list)
		if(filter_.get("eject"))
			go_out()
		if(filter_.get("view_stats"))
			chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
			onclose(chassis.occupant, "msleeper")
			return
		if(filter_.get("inject"))
			inject_reagent(filter_.getType("inject",/datum/reagent),filter_.getObj("source"))
		return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun
	...
	Topic(href,href_list)
		..()
		var/datum/topic_input/filter_ = new (href,href_list)
		if(filter_.get("toggle_mode"))
			mode = !mode
			update_equip_info()
			return
		if(filter_.get("select_reagents"))
			...
			for(var/i=1 to known_reagents.len)
				if(m>=synth_speed)
					break
				var/reagent = filter_.get("reagent_[i]")
			...
			return
		if(filter_.get("show_reagents"))
			chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
		if(filter_.get("purge_reagent"))
			var/reagent = filter_.get("purge_reagent")
			if(reagent)
				reagents.del_reagent(reagent)
			return
		if(filter_.get("purge_all"))

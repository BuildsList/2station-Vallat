...
/obj/mecha/Topic(href, href_list)
	...
	if(href_list["select_equip"])
		var/obj/item/mecha_parts/mecha_equipment/equip = filter_.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rspktoggle"])
		...
		return
	if(href_list["rfreq"])
		var/new_frequency = (radio.frequency + filter_.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		output_access_dialog(filter_.getObj("id_card"),filter_.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		var/mob/user = filter_.getMob("user")
		if(user)
			if(state==0)
				state = 1
			...
			output_maintenance_dialog(filter_.getObj("id_card"),user)
		return
	...
	if (href_list["add_req_access"] && add_req_access)
		operation_req_access += filter_.getNum("add_req_access")
		output_access_dialog(filter_.getObj("id_card"),filter_.getMob("user"))
		return
	if (href_list["del_req_access"] && add_req_access)
		operation_req_access -= filter_.getNum("del_req_access")
		output_access_dialog(filter_.getObj("id_card"),filter_.getMob("user"))
		return

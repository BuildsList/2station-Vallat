...
/obj/machinery/mecha_part_fabricator/Topic(href, href_list)
	...
	if(href_list["part_set"])
		var/tpart_set = filter_.getStr("part_set")
		if(tpart_set)
			if(tpart_set=="clear")
				src.part_set = null
			else
				...
				screen = "parts"
	if(href_list["part"])
		var/list/part = filter_.getObj("part")
		if(!processing_queue)
			build_part(part)
		else
			add_to_queue(part)
	if(href_list["add_to_queue"])
		add_to_queue(filter_.getObj("add_to_queue"))
		return update_queue_on_page()
	if(href_list["remove_from_queue"])
		remove_from_queue(filter_.getNum("remove_from_queue"))
		return update_queue_on_page()
	if(href_list["partset_to_queue"])
		add_part_set_to_queue(filter_.get("partset_to_queue"))
		return update_queue_on_page()
...
*/
	...
	if(href_list["queue_move"] && href_list["index"])
		var/index = filter_.getNum("index")
		var/new_index = index + filter_.getNum("queue_move")
	...
	if(href_list["part_desc"])
		var/obj/part = filter_.getObj("part_desc")
		if(part)

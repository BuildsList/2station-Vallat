			...
			if(copy)
				for(var/i = 0, i < copies, i++)
					if(toner > 0)
						...
						copied = dd_replacetext(copied, "<font face=\"[c.deffont]\" colour=", "<font face=\"[c.deffont]\" nocolour=")	//state of the art techniques in action
						copied = dd_replacetext(copied, "<font face=\"[c.crayonfont]\" colour=", "<font face=\"[c.crayonfont]\" nocolour=")	//This basically just breaks the existing colour tag, which we need to do because the innermost tag takes priority.
						c.info += copied
						c.info += "</font>"

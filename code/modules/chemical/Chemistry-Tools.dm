...
//						T:vessel.trans_to(src, amount) // Virus2 and antibodies aren't in blood in the first place.
	...
	update_icon()
		...
		if(reagents.total_volume)
			...
			var/finalcolour
			for(var/datum/reagent/re in reagents.reagent_list) // natural colour mixing bullshit/algorithm
				if(!finalcolour)
...
/obj/item/weapon/reagent_containers/glass/bottle/insanity
	...
	New()
		...
		reagents = R
		R.my_atom = src
		var/datum/disease/F = new /datum/disease/insanity
		var/list/data = list("virus"= F)
		R.add_reagent("blood", 20, data)

...
turf/proc/add_bloody_footprints(mob/living/carbon/human/M,leaving,d,info,bloodcolour)
	for(var/obj/effect/decal/cleanable/blood/tracks/T in src)
		if(T.dir == d && findtext(T.icon_state, bloodcolour))
	...
	if(leaving)
		this.icon_state = "[bloodcolour][preiconstate]2"
	else
		this.icon_state = "[bloodcolour][preiconstate]1"
	this.dir = d

	if(bloodcolour == "blood")
		this.desc = "These bloody footprints appear to have been made by [info]."
	else if(bloodcolour == "xeno")
		this.desc = "These acidic bloody footprints appear to have been made by [info]."
	else if(bloodcolour == "oil")
		this.name = "oil"

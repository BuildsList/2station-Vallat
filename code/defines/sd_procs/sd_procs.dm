...
colours(colour.dm)
	sd_colour
		...
		VARS
			...
			red			// red componant of the colour
			green		// green componant of the colour
			blue		// red componant of the colour
			html		// html string for the colour
...
HSL procs (hsl.dm)
	hsl2rgb(hue, sat, lgh, scale = 240)
		Returns the RRGGBB format of an HSL colour.
		ALTERNATE FORMAT: hsl2rgb(HSL, scale)

	rgb2hsl(red, grn, blu, scale = 240)
		Returns the HHSSLL string of an RGB colour
...
Nybble colour procs (nybble.dm)
	sd_nybble2rgb(original, bit = 8, short = 0)
		Converts a nybble colour to a hexidecimal RGB colour string.

	sd_rgb2nybble(rgb, bit = 8)
		Converts an RGB colour string to a nybble colour.


Sample sd_colours. (samplecolours.dm)
	This file includes 142 predefined sd_colours. It will not automatically

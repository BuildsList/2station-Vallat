/* Nybble colours
	Nybble colours is used to compact RGB colours to "nybble" (4 bits, 1 hex
	digit, or decimal numbers 0 to 15.) Since BYOND allows up to 16 bits in
	bitwise mathematics, you could store up to 4 colour values in a single
	number. (The project inspiring these procs stores a foreground nybble
	colour, background nybble colour, and 8 bit text character in each 16 bit
	number.)
	...
	Nybble colour values are:
		Dec Hex Bin 	colour
		0	0	0000	null colour. See the note below.
		...
		Null colour note: rgb2nybble() will return a value of 8 for black, so
		that you may use 0 value nybbles for special cases in your own code.
		For example, in the project that inspired these procs, colour 0
...
PROCS
	sd_nybble2rgb(original, bit = 8, short = 0)
		Converts a nybble colour to an RGB hex colour string.
		ARGS:
		...
		RETURNS:
			A 3 or 6 character hexidecimal RGB colour string.

	sd_rgb2nybble(rgb, bit = 8)
		Converts an rgb colour string to a nybble colour.
		ARGS:
			rgb - The colour string to be converted. This may be a 3 or 6
					character colour code with or without a leading "#".
			...
			bit		- the MSB (most significant bit) of the nybble. You can
						...
						DEFAULT: 8
		RETURNS:
			A nybble colour value or null if the proc failed.
*/
...
proc
	...
	sd_rgb2nybble(rgb, bit = 8)
		...
		for(loop = 1 to 3)
			...
			pos += cmp_size
		switch(hi)
			if(0)	return bit	// colour 8: black
